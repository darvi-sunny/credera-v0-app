...
const tasks: Task[] = [
  ...
  { id: 3, title: "Copy files to target", description: "Moving files to the target directory." },
  { id: 4, title: "Sitecore items", description: "Creating sitecore items." },
  { id: 5, title: "Run post-processing", description: "Applying cleanups and optimization." },
  { id: 6, title: "Finish & notify", description: "Wrapping up and sending confirmation." },
];

/** small JSON POST helper */
async function postJson<T = any>(url: string, body: Record<string, unknown>): Promise<ApiResult<T>> {
  try {
    ...
    if (!res.ok) {
      const errMsg = (parsed && (parsed.error || parsed.message)) || `Request failed with status ${res.status}`;
...
async function downloadZip(chatId: string, isView = false) {
  return postJson<{ file?: string; savedTo?: string }>("/api/files/download", { chatId, isView });
}
...
async function copyFiles(extractedPath: string) {
  return postJson<{ extractedPath?: string }>("/api/files/copy", { extractedPath });
}
async function importSitecore(extractedPath?: string) {
  return postJson<{ extractedPath?: string }>("/api/sitecore/import", { filePath: extractedPath });
}
...
function SpinnerBadge() {
  return (
    <span className="inline-flex items-center justify-center h-3 w-3 rounded-full border-2 border-current border-t-transparent animate-spin ml-2" aria-hidden />
  );
}
...
export function AppHeader({ className = "" }: AppHeaderProps) {
  ...
  const [extractedPath, setExtractedPath] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    const id = pathname.startsWith("/chats/") ? pathname.split("/chats/")[1]?.split("/")[0] : undefined;
    setChatId(id);
  }, [pathname]);
  ...
  const pushToSitecore = useCallback(async (isView = false) => {
    if (!chatId) {
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const parsed = text ? JSON.parse(text) : undefined;
        throw new Error(parsed?.error || parsed?.message || `Failed (${res.status})`);
      }
  ...
  return (
    <header className={`${className} border-b border-border dark:border-input`}>
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-4">
            <Link href="/" onClick={handleLogoClick} className="text-lg font-semibold text-gray-900 dark:text-white hover:text-gray-700 dark:hover:text-gray-300">
      ...
      <TaskProgress tasks={tasks} currentStep={currentStep} open={taskModalOpen} onClose={() => setTaskModalOpen(false)} />

      {isCodeViewOpen && extractedPath && (
        <CodeViewer open={isCodeViewOpen} onClose={() => setIsCodeViewOpen(false)} initialZipPath={extractedPath} />
      )}

      <Dialog open={isInfoDialogOpen} onOpenChange={setIsInfoDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold mb-4">v0 Clone Platform</DialogTitle>
          </DialogHeader>

          <div className="space-y-4 text-sm text-gray-600 dark:text-gray-300">
            <p>
              This is a <strong>demo</strong> of a small platform where users generate React components and apps using AI.
            </p>

            <p>
              It's built with{" "}
              <a href="https://nextjs.org" target="_blank" rel="noopener noreferrer" className="underline text-blue-600 dark:text-blue-400">
                Next.js
              </a>
              .
            </p>
          ...
          <div className="flex justify-end mt-6">
            <Button onClick={() => setIsInfoDialogOpen(false)} className="bg-gray-900 dark:bg-gray-100 hover:bg-gray-800 dark:hover:bg-gray-200 text-white dark:text-gray-900">
              Close
            </Button>
          </div>
        </DialogContent>
