...
function buildTree(files: FileItem[]) {
  ...
  for (const f of files) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!cur[part]) {
        cur[part] = { name: part, path: curPath, isFile: i === parts.length - 1, children: {} };
      }
      if (i === parts.length - 1) {
...
const FolderIcon: React.FC<{ open?: boolean }> = ({ open }) => <span style={{ marginRight: 6 }}>{open ? "📂" : "📁"}</span>;
...
const TreeNodeView: React.FC<{
    ...
    return (
      <div style={{ paddingLeft: level * 12, display: "flex", flexDirection: "column" }}>
        <div
          ...
          style={{ cursor: "pointer", display: "flex", alignItems: "center", padding: "4px 6px", borderRadius: 4 }}
        >
          <FileIcon />
  ...
  return (
    <div style={{ paddingLeft: level * 12 }}>
      <div
        onClick={() => setOpen((s) => !s)}
        style={{ display: "flex", alignItems: "center", cursor: "pointer", padding: "6px 4px", userSelect: "none" }}
      >
      ...
      {open && (
        <div style={{ marginTop: 4 }}>
          {Object.keys(node.children)
            ...
            .map((k) => (
              <TreeNodeView key={node.children[k].path} node={node.children[k]} onFileClick={onFileClick} level={level + 1} />
            ))}
        </div>
      )}
    </div>
...
export default function TreeView({ files, onFileClick }: Props) {
  ...
  return (
    <div style={{ fontFamily: "system-ui, Roboto, Inter, -apple-system", fontSize: 13 }}>
      {topKeys.length === 0 ? (
        <div style={{ color: "#666" }}>No files</div>
      ) : (
        topKeys.map((k) => <TreeNodeView key={tree[k].path} node={tree[k]} onFileClick={onFileClick} />)
      )}
    </div>
  );
}
