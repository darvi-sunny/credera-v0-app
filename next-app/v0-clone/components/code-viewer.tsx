...
export default function CodeViewer({ open, onClose, initialZipPath = "" }: Props) {
    ...
    return (
        <div style={{
            ...
            <div style={{ width: "90%", height: "80%", background: "white", borderRadius: 8, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                <div style={{ padding: 12, borderBottom: "1px solid #eee", display: "flex", gap: 8, alignItems: "center" }}>
                    <input value={zipPath} onChange={(e) => setZipPath(e.target.value)} style={{ flex: 1 }} placeholder="Enter absolute path to zip (server allowed base)" />
                    <button onClick={listFiles} disabled={loading}>{loading ? "Listing..." : "List files"}</button>
                    <button onClick={onClose}>Close</button>
                </div>

                <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
                    <div style={{ width: 360, borderLeft: "1px solid #eee", padding: 12, overflow: "auto" }}>
                        {/* Use TreeView here */}
                        <TreeView
                            files={files}
